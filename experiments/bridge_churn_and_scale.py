...
1. Reclustering churn is damped by the dials: with exact conditioning ('siblings',
   'all') the weight change caused by a membership change shrinks to zero as
   (gamma, eta) -> (1, 1). With 'factor' it does not on a generic covariance,
...
for cond in ("siblings", "all", "factor"):
    row = []
...
print("  (exact paths reach zero at (1,1); 'factor' only under a block one-factor model)")
