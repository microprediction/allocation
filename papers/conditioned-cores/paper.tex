...
everything equals the complement against the other cores. Damping that
...
borrowed from the core-orbital allocation of \textcite{hcoa2026}, and the
same model says when that method's strategic use of the core covariance is
accurate.
\end{abstract}
...
That is what we take from \textcite{hcoa2026}. HCOA itself is not an
endpoint of the bridge, and \cref{sec:hcoa} says where it sits.
...
No step forms an $n\times n$ inverse. The largest inverse has dimension
$\max(k-1,\max_i|I_i|)$, as in NCO. The bridge adds work that NCO does not
...
cores are from \textcite{hcoa2026}, and the damping is from
\textcite{cotton2024schur}. What is added is the observation that under a
