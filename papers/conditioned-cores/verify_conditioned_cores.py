...
The repository test suite imports this file and runs the same checks.
  ...
  check_sufficiency      Prop 1: complement against other cores = against everything
  ...
  check_violation        all of the above fail when the orbital model is violated
  check_diagnostic       regressing on other cores false-accepts; R_j does not
  check_change_of_vars   covariance-only rules: apply to M, map back by x = y / b
  check_proxy            Prop 3: V' Sigma V = K Sigma_CC K + D
...
def orbital_model_cov(rng, sizes, violation=0.0):
    """Index 0 of each cluster is its core; sizes[j] orbitals follow.

    violation > 0 correlates orbital residuals across clusters while keeping
    them orthogonal to every core.
    """
    k = len(sizes)
    idx, cores = clusters(sizes)
    n = idx[-1][-1] + 1
...
def cheap_pair(sigma, I, other_cores, gamma, u):
    S_ic = sigma[np.ix_(I, other_cores)]
    coef = gamma * S_ic @ np.linalg.inv(sigma[np.ix_(other_cores, other_cores)])
    return sigma[np.ix_(I, I)] - coef @ S_ic.T, u[I] - coef @ u[other_cores]


def directions(sigma, idx, cores, gamma, u):
    n = sigma.shape[0]
    d = np.zeros(n)
...
def conditioned_cores(sigma, idx, cores, gamma, u):
    n = sigma.shape[0]
    d = directions(sigma, idx, cores, gamma, u)
...
def _random_problem(rng, violation=0.0):
    ...
    return sigma, idx, cores, mu


def check_sufficiency(rng, trials=100, violation=0.0):
    err = 0.0
    for _ in range(trials):
        sigma, idx, cores, mu = _random_problem(rng, violation)
...
def check_bridge_right(rng, trials=100, violation=0.0):
    ...
    for _ in range(trials):
        ...
        for u in (np.ones(len(mu)), mu):
            ...
            stacked = max(stacked, np.abs(directions(sigma, idx, cores, 1.0, u) - target).max())
            w, _ = conditioned_cores(sigma, idx, cores, 1.0, u)
...
def check_bridge_left(rng, trials=100):
    err = 0.0
    for _ in range(trials):
        sigma, idx, cores, _ = _random_problem(rng, violation=0.3)  # no model needed at gamma = 0
        w, _ = conditioned_cores(sigma, idx, cores, 0.0, np.ones(sigma.shape[0]))
...
def check_diagnostic(rng):
    ...
    for name, violation in (("model", 0.0), ("violated", 0.5)):
        sigma, idx, cores = orbital_model_cov(rng, [3, 2, 4], violation)
        coef = R = 0.0
        for j, I in enumerate(idx):
            c, O = I[0], I[1:]
            reg = [c] + [x for l, x in enumerate(cores) if l != j]
        ...
        out[name] = {"other_core_coef": coef, "R": R}
    return out
...
def check_proxy(rng, trials=100):
    err = 0.0
    for _ in range(trials):
        sigma, idx, cores, _ = _random_problem(rng)
        ...
        for i, I in enumerate(idx):
            ...
            D[i] = V[O, i] @ E @ V[O, i]
        rhs = np.diag(kappa) @ sigma[np.ix_(cores, cores)] @ np.diag(kappa) + np.diag(D)
...
def main():
    ...
    assert r["violated"]["other_core_coef"] < TOL  # the regression test false-accepts
