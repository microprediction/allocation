...
portfolio by recursive bisection, consulting only the covariance within
each current block and discarding cross-block information the sample
already contains. We repair this without re-optimizing: calibrate latent
Thurstone abilities that reproduce the hierarchical weights under the
dependence the recursion actually used, then re-race those same scores
under the fuller dependence on hand. Tested on a known-truth synthetic
market and $12{,}000$ real S\&P~500 and REIT trials, the repair reliably
reduces out-of-sample tail risk, with most of the gain coming from the
race's own reallocation rather than from correlation recovery per se.
Whether genuine tail dependence adds further value beyond correlation is
mixed on real data; separated from that estimation question, a
confidence-weighted tail view gives a provably bounded-regret way to act on
a tail belief however it was formed, mirroring the scope of
Black--Litterman's own return views.
\end{abstract}
