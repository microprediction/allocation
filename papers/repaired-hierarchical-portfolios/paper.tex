...
\paragraph{What $C^{\mathrm{used}}$ should be.} This paper tests one family,
the gain-pruned nested tree of \Cref{sec:tree}, indexed by a single
truncation depth. We chose it for tractability: it drops directly into the
existing $k$-factor engine. Two other constructions are worth testing
against it. A flat $K$-cluster block-diagonal reference using the
\emph{exact} (not factor-approximated) within-cluster covariance is more
faithful within each cluster, but requires a separate factorization per
cluster and does not share a rank budget as cleanly across the whole
universe. For Schur specifically, a $\gamma$-blended reference
$C^{\mathrm{used}}(\gamma)=(1-\gamma)C^{\mathrm{tree}}+\gamma\,\hat\Sigma$
would reuse Schur's own coupling parameter directly, rather than requiring
a separate depth sweep to discover the right amount of repair empirically
per $\gamma$. \Cref{sec:gamma-result}'s finding, that the empirically best
depth already tracks $\gamma$, is evidence such a construction is worth
building, not a substitute for building it.
...
independence is the wrong reference on real data. It survives both the
covariance being honestly noisy rather than known, and $12{,}000$ random
trials against real S\&P~500 and REIT sub-portfolios varying every choice
the repair does not control.
