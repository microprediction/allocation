...
knot from each other cluster is a Vecchia approximation, and the
paper gives the rank-one "gateway model" of cross-cluster dependence under
...
- `verify_schur_nco_bridge.py` — certificate for every proposition and
  remark; also run by `tests/test_schur_nco_bridge_certificate.py`
