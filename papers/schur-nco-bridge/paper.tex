...
complement against every other asset. Conditioning instead on one knot from each other cluster is a Vecchia
approximation,
and we give the rank-one model of cross-cluster dependence under which it is
...
set $P_{-i}$ is a Vecchia approximation \parencite{vecchia1988}. The
...
this kind. The gateway model is the conditional independence under which
...
betas, $E_i=\Sigma_{J_iS_i}-\beta_i\beta_i^\top\sigma_{p_i}^2$ for the
