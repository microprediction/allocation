...
indices rather than on everything \parencite{vecchia1988}. The conditionals
...
in the low-rank spatial models of \textcite{banerjee2008} that
\textcite{katzfuss2021} place inside the Vecchia framework. The other
...
A Vecchia approximation \parencite{vecchia1988} orders the variables and
conditions each on a small subset of its predecessors. The conditionals are
Schur complements, and the approximation is the choice of conditioning
sets. The gateway model is one such choice made exact: the knots come
first, jointly, and each cluster's remaining members are conditioned on
their own knot alone. It is a sparser relative of the knot-based special
cases collected by \textcite{katzfuss2021}.

\Cref{prop:sufficient} is the Markov property of that graph. Given the other
knots, a cluster is uncorrelated with every other asset outside it. The
conditioning here is on a subset of all other variables rather than of
predecessors, because only the blocks of $\Sigma^{-1}u$ are needed and no
joint density is formed. When the model fails, using \eqref{eq:cheap}
anyway is the approximation.
\end{remark}

\begin{remark}[Testing the model and choosing knots]
Regressing each remaining member on its own knot and the other knots, and checking
that the other-knot coefficients vanish, does not test the model. Member
residuals can be orthogonal to every knot and still be correlated with each
other across clusters, and the sufficiency of the knots then fails. The
direct diagnostic is the size of $R_j(p_j)$ in \eqref{eq:residual},
computed from the sample covariance before any portfolio is built. The same
quantity gives a rule for choosing knots: take $p_j$ to minimize
$\|R_j(p)\|$ over $p\in I_j$.
\end{remark}
...
the factor. The member most aligned with the cluster's first principal
component is then approximately the right knot.
\end{remark}
...
complement in \eqref{eq:complement}, as any Vecchia approximation does. The
size of $R_j$ in \eqref{eq:residual} measures what is lost.

\section{Closing}

The contribution is deliberately small.\footnote{A script next to the
manuscript, \path{verify_schur_nco_bridge.py}, checks
\cref{prop:sufficient,prop:bridge,prop:outer}, the remarks on diagnostics,
on covariance-only rules and on the tree, and the symmetric-factor formula,
on random covariances to $10^{-10}$. It also confirms that the exactness
claims fail when the gateway model is violated. The repository's test suite
runs it.} The identity is block inversion, the
restricted conditioning set is Vecchia's, and the damping is from
\textcite{cotton2024schur}. What is added is the model under which one knot
