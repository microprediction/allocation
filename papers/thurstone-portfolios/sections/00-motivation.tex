...
We develop \emph{Thurstone portfolio polishing}: a way to take an existing
allocation on the simplex---a capitalization index, an equal-weight book, any
benchmark---and re-tilt it under a view on \emph{dependence} rather than returns.
The vehicle is a race among assets: each asset draws a noisy performance, and a
weight is the probability that it comes out best. The abilities driving the race
are backed out of the benchmark by a fast \emph{exact} inverse, and the polish is
what happens when those same abilities are re-raced under a richer dependence law.
The race has a second reading as a Thurstonian model of choice, which we set against
...
single property---\emph{clone consistency}: cloning an option should not create
value. The same axiom is named in social choice, where it is Tideman's
\emph{independence of clones} \parencite{tideman1987}. Add a second asset that
behaves almost exactly like one already held, and a
sensible rule should \emph{split} the position across the two near-duplicates and
leave the rest of the book untouched; it must not let the duplicated exposure count
twice. Everything else this paper advertises---robustness without inverting a
covariance, low turnover, feasibility at scale---is downstream of meeting this one
axiom with a smooth order-statistic race rather than a hard clustering or an inverse. This is the portfolio form of
the red-bus/blue-bus problem that unseated the logit model in discrete choice
\parencite{mcfadden1974}. The logit inherits that paradox from its noise:
independence of irrelevant alternatives holds exactly when the race is driven by
Gumbel errors \parencite{yellott1977}, so a race under any other law escapes it by
construction. A Lucian rule---capitalization weighting included---fixes
...
of \Cref{prop:restriction}: it endorses cap weighting on the whole-market universe
alone, and is silent, indeed contrary, on the restricted universes where weighting
decisions are actually made. This is the
