...
standard-normal ensemble by $C_{\mathrm{tilt}}^{1/2}$ (a randomized Sobol'
quasi-Monte-Carlo sequence when available); a Student-$t$ and a low-rank factor
race (for scale) ship alongside; and any user callable obeying the fixed-seed
contract supplies an arbitrary law. Tail consistency (\Cref{prop:tail}) follows by
