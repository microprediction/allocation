...
Numerically the contrast is stark: as the reference correlation is driven toward
singularity---condition number into the hundreds---the choice-sensitivity
$\nabla_\theta w$ stays uniformly scaled, its own condition number remaining near
one, so the displaced inverse never inherits the ill-conditioning that defeats
$\Sigma^{-1}$, and a Newton calibration step preconditioned by it converges in a
...
a simulated path crosses a tie boundary---so it is not literally Lipschitz. Under
fixed seeds and a bounded tie-boundary density it is low-turnover only in
expectation,
\begin{equation*}
  \E\,\|\widehat w_M(C+\Delta C) - \widehat w_M(C)\|_1
  \;=\; O(\|\Delta C\|_F) \,+\, O(1/\sqrt M),
\end{equation*}
the first term the population motion of \eqref{eq:lip} and the second a Monte-Carlo
floor from paths near a boundary. The finite-$M$ map inherits \emph{low turnover in
expectation}, not smoothness---the honest content behind the common-seed transport.
