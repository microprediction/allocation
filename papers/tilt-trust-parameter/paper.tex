...
under one correlation, re-evaluate the same abilities under another, and the
weights move toward whatever the first law was missing. Nothing is inverted at
any point, so the construction is available where a covariance is singular.

Two things govern what it is worth and they are independent. The first is how
much correlation the benchmark already accounts for. The race halves the
variance of equal weight and of inverse variance and moves a long-only
minimum-variance portfolio by a tenth of a percent, so it repairs what needs
repairing and is nearly a fixed point on what does not. The second is whether
the estimated correlation deserves trust, which is what the damping expresses:
at a tenth of an observation per asset a full tilt costs nineteen percent, at
five observations per asset it saves thirteen, and the optimum walks
monotonically between.

Hierarchical portfolios are the leading application, because recursive
bisection discards cross-block covariance by construction rather than by
judgement. On a known-truth contagious market the tilt cuts out-of-sample
ES95 by $9$ to $17$ percent against plain HRP and Schur. Across $12{,}000$
random trials on real S\&P~500 and REIT sub-portfolios it wins $59$ to $80$
percent of the time under every covariance estimator tried, and the largest
tilt, calibrated against an independent reference, is the single best setting.

Two results point away from the obvious extensions. A tail-aware Student-$t$
race is consistently worse than the Gaussian one, on both variance and
shortfall, on a market built to have asymmetric tail dependence. And the
operator should be applied once: it is not a fixed point of itself, and
iterating drives a weight to zero within three to six rounds. Separated from
the estimation question, a confidence-weighted tail view is bounded-regret:
zero confidence reproduces the correlation-only tilt exactly and the cost of a
wrong belief grows smoothly with confidence rather than catastrophically.
\end{abstract}

\section{Introduction}

A long-only portfolio and a vector of latent abilities carry the same
information. Calibrate the abilities that reproduce a portfolio under one
dependence law, re-evaluate those same abilities under another, and the
portfolio has moved toward what the first law was missing. No optimization is
re-run and no covariance is inverted, so the operation is available where a
covariance is singular and cheap enough to sweep. This paper measures what it
is worth, and when.

Two things govern that and they are independent of each other. How much
correlation the benchmark already accounts for bounds what is available at
all: \Cref{sec:benchmark} applies one race to three benchmarks spanning the
range and finds it halves the variance of the two that ignore correlation
while moving a long-only optimum by a tenth of a percent. Whether the
estimated correlation deserves trust decides how far to go, and
\Cref{sec:dial} shows that dial behaving as a trust parameter should, with its
optimum walking from zero to one as coverage grows.

Hierarchical portfolios are the leading application and the rest of the paper
is about them, because they are the case where the benchmark's blindness is
...
The paper makes five claims. First, that racing a benchmark under a
different dependence law is a diversification operator, and that two
independent things govern what it is worth: how much correlation the benchmark
already accounts for, which bounds what is available, and whether the
estimated correlation deserves trust, which the damping expresses.
\Cref{sec:benchmark,sec:dial} establish each on its own. Second, that the race
can be evaluated exactly, by quadrature rather than simulation, so the
identity at zero tilt holds to $10^{-10}$ and is a check to assert rather than
a property to assume. Third, a constructive object for ``the dependence a
hierarchical bisection actually consulted'': a nested factor model with one
latent factor per retained split of the same median-bisection tree HRP and
Schur build, which drops into the $k$-factor calibration engine of
\textcite{cotton2026factorprobit} with no new machinery and makes hierarchical
portfolios the leading application. Fourth, the evidence: a known-truth
contagious market on which the tilt is tested against plain HRP and Schur
across the amount of tree retained, Schur's $\gamma$ and the sample size, and
a replication across $12{,}000$ random trials on S\&P~500 and REIT
sub-portfolios varying every choice the tilt does not control. Most of the
gain is the race's own reallocation rather than correlation recovery, and the
largest tilt, calibrated against an independent reference, is the best
setting on real data.

Fifth, what did not work. A tail-aware Student-$t$ race is worse than the
Gaussian one on both variance and shortfall, on a market built to have
asymmetric tail dependence. The operator is not a fixed point of itself and
should be applied once. And every comparison here is one-shot: the one result
re-checked under a walk-forward protocol kept its turnover cost and lost most
of its edge, which \cref{sec:scope} takes up.
...
\begin{remark}[The identity at zero is a check, not a property]
$T_{S_0\to S_1}$ reproduces its benchmark exactly when $S_1 = S_0$, and that
has to be asserted rather than assumed. Calibrating under one law and
evaluating at nominal zero tilt under another leaves the origin of the dial
already tilted, so the whole curve is measured from the wrong place and every
number on it is wrong by an amount that does not announce itself. Four
attempts at this construction failed exactly that way before one did not: a
reference calibrated under independence but raced under the full correlation,
a target approximated by its leading eigenvectors, which are the global
factors and so delete the block structure instead of adding to it, and two
variants of the same. Assert $\|T_{S_0\to S_0}(w^0) - w^0\|_1 \le 10^{-10}$
before reading any other number.
\end{remark}

\paragraph{Evaluating the race.} $W_S$ can be computed by quadrature over the
field or by simulating the race and counting wins, and the difference is not a
detail. A simulation's resolution tracks paths \emph{per asset}: at five
thousand names and four thousand paths there is less than one path per asset,
most names never win once, and the returned book differs from its own input by
eighty-two percent. Quadrature reproduces the benchmark at zero tilt to
$10^{-10}$ in a tenth of a second, where the simulation needs sixty-five
thousand paths and 2.6 gigabytes to reach $0.2$. The damping also stays inside
the factor family, since
\[
  (1-\phi) I + \phi \big( V V^\top + \operatorname{diag}(D) \big)
  = \big(\sqrt{\phi}\,V\big)\big(\sqrt{\phi}\,V\big)^\top
    + \operatorname{diag}\big((1-\phi)\mathbf 1 + \phi D\big),
\]
so every point of the dial is itself an exact $k$-factor race and the blend
introduces no approximation of its own. Quadrature is used throughout except
where a Student-$t$ law is raced, which has no quadrature equivalent.
...
asset list. At each split into halves $L,R$, the $\gamma$-damped Schur
complement of each half's own block,
\[
  \Sigma_{LL}^c(\gamma) = \Sigma_{LL} - \gamma\,\Sigma_{LR}\Sigma_{RR}^{-1}\Sigma_{RL}
  \quad(\text{and symmetrically for } R),
\]
is collapsed into the augmented block actually used for the split,
\[
  \Sigma_{LL}^{\mathrm{aug}} = \mathrm{sym}\Big[\big(I - \gamma\,\Sigma_{LR}\Sigma_{RR}^{-1}\big)^{-1}\Sigma_{LL}^c(\gamma)\Big],
\]
...
both the damped Schur complement and its collapse reduce to $\Sigma_{LL}$
itself, $\Sigma_{LR}$ is never read, and the recursion is exactly
Hierarchical Risk Parity. Every off-diagonal entry of $\Sigma$ is consulted
only while its two assets remain in the same block. The instant they are
placed in different halves, their covariance is never read again (at
$\gamma=0$) or read only through the damped Schur complement and its
collapse (at $\gamma>0$).

\paragraph{Two-stage cluster methods.} Nested clustered optimization
\parencite{lopezdeprado2020} replaces the bisection tree with one flat
partition into clusters. Each cluster is allocated internally from its own
block, and a $k\times k$ problem across the cluster portfolios sets the
cluster budgets. The within-cluster step reads no cross-cluster covariance,
so the information discarded is the same as at an HRP split, once rather
than recursively. The repair operator below applies once
$C^{\mathrm{used}}$ is specified for a flat partition; we do not pursue
that here. \textcite{cotton2026ncobridge} gives a Schur bridge from NCO to
unconstrained minimum variance that conditions each cluster on one
representative of every other cluster.
...
\subsection{What the tilt is worth depends on what you hand it}
\label{sec:benchmark}

The operator is a diversification operator, and the first thing that governs
its value is how much correlation its benchmark already accounts for. Before
turning to hierarchical portfolios, \Cref{tab:benchmark} applies the same race,
under the same correlation, to three starting points on a one-factor market
...
\bottomrule
\end{tabular}
\caption{One race, one correlation, three starting points, $n=120$. The
long-only optimum is solved, not approximated.}
\label{tab:benchmark}
\end{table}

It halves the variance of the two benchmarks that ignore correlation and
leaves the optimum where it is. That is the behaviour a correct operator
should have: it repairs what needs repairing and is nearly a fixed point on
what does not. The direction is right for the same reason. Starting from equal
weight on a market with loadings spread from $0.1$ to $0.9$, the raced weights
correlate $-0.996$ with the loading against $-0.989$ for long-only minimum
variance, and the race captures eighty-four percent of the variance reduction
available between equal weight and the optimum. Assets that move with the
field are less often uniquely the best, so the race discounts them, which is
what diversification asks for and is reached without inverting anything.

This is why hierarchical portfolios are the right application and also bounds
what is available from them. Recursive bisection discards cross-block
covariance by construction, so there is something left for the race to act on.
An allocator that already consults the whole covariance has nothing to gain,
and at five thousand assets, where the hierarchical rule is the best
construction available, tilting it makes it monotonically worse, by $3.2$
percent at full tilt across ten draws. The operator is not failing there. It
has nothing to add.

\subsection{And on whether the estimate deserves trust}
\label{sec:dial}

The second governor is independent of the first. Damping the target law toward
the reference by a parameter $\phi$ interpolates between the benchmark and a
full race under the estimate, and how far to go is a question about the
estimate rather than about the benchmark. \Cref{tab:dial} sweeps it on the
crash market of \Cref{sec:market}, tilting a hierarchical portfolio, paired on
the same seed ensemble so what remains is the effect of the dial.
...
\bottomrule
\end{tabular}
\caption{Full tilt against no tilt, paired, forty assets, three seeds. The
ratio is the variance at $\phi=1$ over the variance at $\phi=0$, so below one
is an improvement.}
\label{tab:dial}
\end{table}
...
in roughly nine draws out of ten. Expected shortfall follows the same shape
rather than a different one, neutral at parity and $80$ to $84$ percent once
there is real data.

The two governors multiply. A benchmark with nothing left to fix gives nothing
at any coverage, and an untrustworthy estimate costs regardless of how much
room the benchmark has.
...
We built and tested the $\gamma$-blended reference. $C^{\mathrm{tree}}$ is taken as the full retained
...
\subsection{A confidence-weighted tail view}
\label{sec:tailbelief}

\Cref{sec:main-result} and \Cref{sec:realmarket} both asked whether the
repair can \emph{discover} tail dependence from ordinary historical data,
by fitting or resampling a tail law and re-racing under it. The answer was
no: a fitted degrees-of-freedom parameter is badly biased at realistic
sample sizes, and a bootstrap of historical returns has nothing to work
with when the window contains no example of the crash it would need to
anticipate. That is a genuine limit on \emph{estimating} a tail view from
data. It is a different question from whether the repair gives a safe way
to \emph{act} on a tail view an investor already holds, however that view
was formed. Black--Litterman does not claim to generate good return
views either; it claims to let an investor act coherently on ones they
already hold, anchored to a benchmark, with a confidence parameter
controlling how far to depart from it. The same structure applies here,
and two of its properties are provable without knowing whether the view is
correct.

First, the fixed-point property already established for the repair itself
extends directly to a confidence-weighted tail view: at zero confidence the
construction reproduces the correlation-only repair exactly, before any
tail belief is applied. Second, the smoothness theorem and the local
Bregman expansion in \textcite{cotton2026polish}'s reverse-optimization
section together give a departure from that anchor that grows linearly in
the confidence weight $\phi$, at a cost that grows quadratically. A
cautious investor who is unsure whether their tail belief is right can cap
its worst-case damage by choosing $\phi$ small, while still capturing a
proportional share of the benefit if the belief turns out to be right.
Raising $\phi$ does not provably improve realized performance in general:
whether it does depends on whether the belief is correct, exactly the
discovery problem \Cref{sec:main-result} and \Cref{sec:realmarket} showed
is hard.

\paragraph{Construction.} Let $w^{\mathrm{gauss}}$ be the correlation-only
repair of \Cref{sec:main-result} and $w^{\mathrm{tail}}$ the same repair
re-raced under a supplied tail law (the same cluster-specific Student-t
race used in \Cref{sec:main-result}'s known-truth market, one degree-of-freedom
parameter per correlated cluster). The confidence-weighted view is
\[
  w^\phi = \mathrm{normalize}\big((1-\phi)\,w^{\mathrm{gauss}} + \phi\,w^{\mathrm{tail}}\big),
  \qquad \phi\in[0,1],
\]
the same linear-blend construction already used for the correlation dial
in \textcite{cotton2026polish}. $\phi=0$ recovers $w^{\mathrm{gauss}}$
exactly; $\phi=1$ is full confidence in the supplied tail view.

\paragraph{A correct belief.} \Cref{tab:tailbelief} sweeps $\phi$ on the
known-truth cluster-tail market of \Cref{sec:main-result} ($\nu=2.2$,
supplied correctly), $400$ trials.

\begin{table}[h]
\centering
\caption{ES99 gain over $w^{\mathrm{gauss}}$ as confidence $\phi$ rises,
correct belief ($\nu=2.2$ true and supplied), $400$ trials.}
\label{tab:tailbelief}
\footnotesize
\begin{tabular}{lccccc}
\toprule
$\phi$ & $0.00$ & $0.25$ & $0.50$ & $0.75$ & $1.00$ \\
\midrule
mean gain & $0$ & $0.0117$ & $0.0200$ & $0.0248$ & $0.0260$ \\
win rate  & --- & $68.2\%$ & $64.2\%$ & $61.3\%$ & $57.2\%$ \\
\bottomrule
\end{tabular}
\end{table}

The gain rises monotonically and never turns negative, with a diminishing-returns
shape and a win rate that stays clearly above half at every tested $\phi$.

\paragraph{A wrong belief.} The same sweep, but the market is actually
Gaussian (no tail dependence at all) while the investor still supplies
$\nu=2.2$, checks the other side of the guarantee.

\begin{table}[h]
\centering
\caption{ES99 cost relative to $w^{\mathrm{gauss}}$ as confidence $\phi$
rises, wrong belief (market is Gaussian, $\nu=2.2$ supplied anyway), $400$
trials.}
\label{tab:tailwrong}
\footnotesize
\begin{tabular}{lccccc}
...
$\phi$ & $0.00$ & $0.25$ & $0.50$ & $0.75$ & $1.00$ \\
\midrule
mean cost & $0$ & $-0.0046$ & $-0.0118$ & $-0.0214$ & $-0.0333$ \\
win rate  & --- & $14.2\%$ & $10.5\%$ & $6.0\%$ & $3.2\%$ \\
\bottomrule
\end{tabular}
\end{table}

Being wrong costs something, and the cost grows with confidence exactly as
the theory predicts: smoothly, with no jump, and controllable by choosing
$\phi$. The magnitude at full confidence ($-0.0333$) is the same order as
the gain at full confidence when the belief is correct ($+0.0260$): a
symmetric bet, not a catastrophic asymmetric one.

\paragraph{An imprecise belief.} A real investor rarely knows the exact
degrees of freedom of a tail law; \Cref{tab:tailnu} holds $\phi=1$ fixed
and sweeps the supplied $\nu$ against the true market $\nu=2.2$, $400$
trials per row.

\begin{table}[h]
\centering
\caption{ES99 gain at full confidence ($\phi=1$), true market $\nu=2.2$,
varying the SUPPLIED $\nu$, $400$ trials per row.}
\label{tab:tailnu}
\footnotesize
\begin{tabular}{lccccc}
\toprule
supplied $\nu$ & $2.2$ (exact) & $3.0$ & $5.0$ & $10.0$ & $30.0$ \\
\midrule
mean gain & $0.0173$ & $0.0453$ & $0.0629$ & $0.0587$ & $0.0561$ \\
win rate  & $55.8\%$ & $68.2\%$ & $70.0\%$ & $69.5\%$ & $70.2\%$ \\
\bottomrule
\end{tabular}
\end{table}

Every supplied $\nu$ tested helps, including $30$, barely fat-tailed at
all against a true $2.2$: knowing the qualitative direction of the belief
matters far more than knowing its exact strength. The moderate values
($5$--$10$) outperform the exact true value here, plausibly because an
aggressively fat-tailed supplied law interacts riskily with the ordinary
estimation noise already present in $\theta$ and the correlation estimate,
while a more moderate stance captures most of the real effect with less of
that added fragility.

\paragraph{Scope.} An investor who already believes there is tail
dependence, from sector knowledge, structural reasoning, or judgment the
data alone would not reveal, has a coherent, boundedly safe way to act on
that belief here, with $\phi=0$ always available as a fallback to the
correlation-only repair and no requirement that the belief be
statistically validated first. Discovering tail structure from ordinary
historical data remains hard, exactly as \Cref{sec:main-result} and
\Cref{sec:realmarket} found; this section leaves that finding in place.
...
Four limits, one of which bears on every number above.

\paragraph{The protocol is one-shot.} Every comparison in this paper uses a
cold-start, fresh-seed protocol: each trial builds its allocation from scratch
and is scored once. That is not the streaming, common-seed-transport regime
the smoothness of these estimators is actually about. The one result we
re-checked under a walk-forward protocol saw its edge over simple alternatives
largely evaporate while its turnover cost, two to three times higher, did not.
A one-shot protocol can overstate the case, and the size of the effects
reported here should be read with that discount until they are re-measured
under a fair walk-forward.

\paragraph{The round trip is not exact everywhere.} The solver used for the
inverse enters a limit cycle on small, highly concentrated fields, and neither
more iterations nor a tighter tolerance nor finer quadrature moves it. A damped
...
\paragraph{Where the operator does nothing is measured narrowly.} That it is a
fixed point on an already-diversified benchmark is established on a one-factor
market and on a hierarchical portfolio at five thousand assets. Whether it is a
fixed point on other optima is untested.

\paragraph{The synthetic markets are simulated.} The population covariance has
to be known for realized risk to be exact and for the elliptical check to be
possible at all. The real-market trials of \Cref{sec:realmarket} are the
counterweight, and they are scored out of sample rather than against a known
truth.
...
That null result is about \emph{discovering} tail dependence from data, not
about acting on a belief already held. Separated from the estimation
question, a confidence-weighted tail view has a provable safety property
independent of whether the belief is correct: it reproduces the
correlation-only repair exactly at zero confidence, and the cost of being
wrong grows smoothly and boundedly with how much confidence is placed in
it, never catastrophically. An investor who believes there is tail
dependence, for reasons the data alone would not reveal, can act on that
belief coherently without first winning the harder argument this paper's
own tail-dependence checks could not settle.
