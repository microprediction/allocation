...
The race is evaluated exactly, by quadrature over the field, rather than
simulated. That is not a detail. A simulated race assigns weight by counting
wins, so its resolution tracks paths per asset: at five thousand names and
four thousand paths there is less than one path per asset, most names never
win once, and the returned book differs from its own input by eighty-two
percent. Quadrature reproduces the benchmark at zero damping to $10^{-10}$ in a
tenth of a second, where the simulation needs sixty-five thousand paths and
2.6 gigabytes to reach $0.2$.
...
\section{And on whether the correlation deserves trust}
