...
| race + regime | those plus the crash law: probability, depth, dispersion | Thurstone under the true mixture law |
...
| oracle | the true sub-covariance and the market's returns | unavailable; maximum Sharpe |
| tail oracle | scenarios from the true law | unavailable; minimum expected shortfall per unit return |
...
## Two laws, one covariance

`--law gaussian` is the sector market as a Gaussian copula. `--law regime` is
the same covariance generated by the law equities actually follow: a market
crash day about five times a year on which everyone falls modestly and
dispersion collapses, a rarer day on which one whole sector falls, and a
common volatility regime. Each layer is built so the total covariance is
unchanged, so `Sigma`, `m`, Black-Litterman and the Sharpe oracle are
identical under both laws and only the copula differs. The calibration is in
`markets.py`, against the S&P 500 panel: the frequency with which all thirty
names of a random basket fall together, the excess of lower and upper tail
dependence over a Gaussian copula, and the frequency with which half of a
sector sits in its residual tail on one day.

Rules are scored twice: Sharpe against `(Sigma, m)`, which the law cannot
change, and expected shortfall at 95 percent under the true law, which it can.
Black-Litterman is a second-moment method and sees the same covariance either
way. The race can be handed the mixture: `race+regime` runs
`(1 - p) race_normal + p race_crash` and calibrates on the blend, so the
question the regime law asks is whether a method that can represent the tail
beats the correctly specified second-moment method on the tail metric.
...
LAW=regime SUBSET=sector ./launch.sh index   # the tail study
