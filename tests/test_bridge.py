...
# ------------------------------------------------------------------ corners
@pytest.mark.parametrize("conditioning", ["tree", "flat"])
...
def test_full_coupling_does_not_depend_on_the_partition():
    ...
    ws = [bridge_weights(cov, contiguous_partition(rng.permutation(10), k), gamma=1.0, eta=1.0,
                         conditioning="flat") for k in (2, 3, 4)]
...
def test_hrp_is_gamma_zero_with_naive_fitness():
    ...
    w = bridge_weights(cov, bisection_tree(order), gamma=0.0, eta=1.0, fitness="naive")
...
def test_herc_is_the_zero_corner():
    ...
    ref /= ref.sum()
    assert _close(bridge_weights(cov, part, gamma=0.0, eta=0.0, conditioning="flat"), ref)
...
def test_nco_is_gamma_zero_with_the_outer_optimizer():
    ...
    ref = V @ a
    ref /= ref.sum()
    assert _close(bridge_weights(cov, part, gamma=0.0, eta=1.0, conditioning="flat", outer="optimize"), ref)
...
def test_stevens_path_from_inverse_variance_to_minimum_variance():
    ...
    ivp /= ivp.sum()
    one = [np.arange(9)]
    assert _close(bridge_weights(cov, one, gamma=0.0, eta=0.0, conditioning="flat"), ivp)
    assert _close(bridge_weights(cov, one, gamma=0.0, eta=1.0, conditioning="flat"), min_variance_weights(cov))
    single = [np.array([i]) for i in range(9)]
    assert _close(bridge_weights(cov, single, gamma=0.0, eta=0.0, conditioning="flat"), ivp)
    assert _close(bridge_weights(cov, single, gamma=1.0, eta=0.0, conditioning="flat"), min_variance_weights(cov))
...
def test_long_only_option_caps_eta_and_keeps_weights_nonnegative():
    ...
    w, info = bridge_weights(cov, part, gamma=0.0, eta=1.0, conditioning="flat", long_only=True, return_info=True)
...
def test_knots_are_exact_under_a_block_one_factor_model(outer):
    ...
    w = bridge_weights(S, clusters, gamma=1.0, eta=1.0, conditioning="knots", outer=outer)
...
def test_knots_are_close_on_a_generic_covariance_and_continuous_in_gamma():
    ...
    cov = _cov(12, rng)
    part = contiguous_partition(rng.permutation(12), 3)
    w1 = bridge_weights(cov, part, gamma=1.0, eta=1.0, conditioning="knots")
    assert np.max(np.abs(w1 - min_variance_weights(cov))) < 0.1
    prev = None
    for g in np.linspace(0, 1, 11):
        w = bridge_weights(cov, part, gamma=float(g), eta=1.0, conditioning="knots")
        if prev is not None:
...
def test_estimator_nests_named_methods():
    ...
    hrp = SchurBridge(gamma=0.0, fitness="naive").fit(X)
...
def test_estimator_fixed_labels_and_partial_fit_are_smooth():
    ...
    est = SchurBridge(gamma=0.5, eta=0.5, clusters=labels, conditioning="knots", halflife=100.0)
    est.fit(X[:200])
...
def test_streaming_bridge_fixed_clusters_and_mean_companion():
    ...
    est = StreamingSchurBridge(gamma=1.0, eta=1.0, clusters={"a": "x", "b": "x", "c": "y", "d": "y"},
                               conditioning="knots", companion="mean", min_obs=5)
    for _ in range(40):
