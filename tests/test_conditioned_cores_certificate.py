"""Run the certificate for papers/conditioned-cores under pytest."""
...
SCRIPT = Path(__file__).resolve().parents[1] / "papers" / "conditioned-cores" / "verify_conditioned_cores.py"

pytestmark = pytest.mark.skipif(not SCRIPT.exists(), reason="conditioned-cores paper not present")


@pytest.fixture(scope="module")
def cert():
    spec = importlib.util.spec_from_file_location("verify_conditioned_cores", SCRIPT)
...
def test_cores_are_sufficient(cert, rng):
...
def test_other_core_regression_false_accepts_but_R_does_not(cert, rng):
    r = cert.check_diagnostic(rng)
    assert r["model"]["R"] < cert.TOL
    assert r["violated"]["other_core_coef"] < cert.TOL
...
def test_core_covariance_is_systematic_part(cert, rng):
